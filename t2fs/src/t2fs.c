...
int read2(FILE2 handle, char *buffer, int size) {
	...
	if (_opened_file_[handle].curr_pointer + size
			> _opened_file_[handle].record.bytesFileSize) {
		bytes_to_read = _opened_file_[handle].record.bytesFileSize
	...
	while (bytes_left > 0) {
		DWORD current_block = _opened_file_[handle].curr_pointer
				/ _super_block_.BlockSize;
		DWORD current_offset = _opened_file_[handle].curr_pointer
				% _super_block_.BlockSize;
		BYTE block[_super_block_.BlockSize];
		//reads block from disk
		if (read_file_block(_opened_file_[handle].inode, current_block, block,
				_super_block_.BlockSize) < 0)
			return -1;
...
int write2(FILE2 handle, char *buffer, int size) {
	...
	if (_opened_file_[handle].record.bytesFileSize
			< size + _opened_file_[handle].curr_pointer) {
		//blocks need to be allocated
		increase_in_size_bytes = size + _opened_file_[handle].curr_pointer
				- _opened_file_[handle].record.bytesFileSize;
	}
	...
	DWORD curr_pointer_block = _opened_file_[handle].curr_pointer
			/ _super_block_.BlockSize;
	//ofset from the start of the block
	DWORD curr_pointer_offset = _opened_file_[handle].curr_pointer
			% _super_block_.BlockSize;
	...
	if (curr_pointer_offset > 0) {
		...
		if (read_file_block(file_inode_addr, curr_pointer_block, block_buffer,
				_super_block_.BlockSize) != 0) {
			return -1;
		}
		...
		//writes block back
		if (write_file_block(file_inode_addr, curr_pointer_block, block_buffer)
				!= 0) {
			return -1;
		}
	...
	while (bytes_to_write > 0) {
		...
		curr_pointer_block = _opened_file_[handle].curr_pointer
				/ _super_block_.BlockSize;
		curr_pointer_offset = _opened_file_[handle].curr_pointer
				% _super_block_.BlockSize;
		int bytes_to_copy;
		if (bytes_to_write < _super_block_.BlockSize - curr_pointer_offset) // in case of not writing until the end
				{ //of the block
		...
		if (curr_pointer_block + 1
				> _opened_file_[handle].record.blocksFileSize) {
			...
			if (write_file_block(file_inode_addr, curr_pointer_block,
					block_buffer) != 0) {
				return -1;
			}
			...
			if (read_file_block(file_inode_addr, curr_pointer_block,
					block_buffer, _super_block_.BlockSize) != 0) {
				return -1;
			}
			memcpy(block_buffer, buffer, bytes_to_copy);
			if (write_file_block(file_inode_addr, curr_pointer_block,
	...
	int record_position = find_record(&dummy, parent_inode,
...
int readdir2(DIR2 handle, DIRENT2 *dentry) {
	...
	case TYPEVAL_DIRETORIO:
		dentry->fileType = 1;
		dentry->fileSize = inode_dir_size_blocks(record.i_node)
				* _super_block_.BlockSize;
		break;
